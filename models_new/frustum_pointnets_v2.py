...
def get_model(point_cloud, one_hot_vec, is_training, bn_decay=None):
   ...
   #############Invariance transformation Net###########################################
    ...
    edge_net =  tf_util.conv2d_dgcnn(edge_net,128,[1,1],padding = 'VALID',stride = [1,1], bn=True, is_training=is_training,bn_decay= bn_decay, scope = "edge_conv_2" )
    edge_net = tf.reduce_max(edge_net, axis = -2, keep_dims = True)
    ...
    with tf.variable_scope('feature_transform_net') as sc:
        transform = input_transform_net(edge_feature_edge,is_training,bn_decay,K=3)
