...
class Invariance_Transformation_Net:

    def __init__(self, point_cloud, features, is_training, invarians_trans_param):
        ...
        if hasattr(invarians_trans_param,'xdconv_params'):
            for layer_idx, layer_param in enumerate(invarians_trans_param.xdconv_params):
                ...
                pts=self.layer_pts[0]
                fts=self.layer_fts[-1]
                qrs=self.layer_pts[0]
                fts_qrs=self.layer_fts[-1]
                ...
                fts_fuse=pf.dense(fts_concat,C_pts_fts,tag+'fts_fuse',is_training)
                self.layer_pts.append(qrs)
        ...
        for layer_idx,layer_param in enumerate(fc_params):
            ...
            #print((self.fc_layers[0]))
            #print(len(self.fc_layers))

        print('pts_layers',type(self.layer_pts))
        print('fcs_layers',type(self.layer_fts))
        print('pts_layers_len',len(self.layer_pts))
        print('fcs_layers_len',len(self.layer_fts))
        print('pts_layers_len',type(self.layer_pts[-1]))
        print('fcs_layers_len',type(self.layer_fts[-1]))
        #print('pts_layers_-1',self.layer_pts[-1].get_shape())
        #print('fcs_layers_-1',self.layer_fts[-1].get_shape())
        #print('fcs_layers_-2',self.layer_fts[-2].get_shape())
        #print('fcs_layers_-3',self.layer_fts[-3].get_shape())
        #print('fcs_layers_-4',self.layer_fts[-4].get_shape())
        #print('fcs_layers_1',self.layer_fts[1].get_shape())
        #print('fcs_layers_2',self.layer_fts[2].get_shape())
