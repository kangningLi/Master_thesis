...
def conv2d_dgcnn(inputs,
  ...
  with tf.variable_scope(scope) as sc:
      ...
      if bn:
        outputs = batch_norm_for_conv2d_dgcnn(outputs, is_training, bn_decay=bn_decay, scope='bn')
...
def fully_connected_dgcnn(inputs,
                    num_outputs,
                    scope,
                    use_xavier=True,
                    stddev=1e-3,
...
def batch_norm_template_dgcnn(inputs, is_training, scope, moments_dims, bn_decay):
  """ Batch normalization on convolutional maps and beyond...
  Ref.: http://stackoverflow.com/questions/33949786/how-could-i-use-batch-normalization-in-tensorflow
...
def batch_norm_for_conv2d_dgcnn(inputs, is_training, bn_decay, scope):
    return batch_norm_template_dgcnn(inputs, is_training, scope, [0,1,2], bn_decay)
